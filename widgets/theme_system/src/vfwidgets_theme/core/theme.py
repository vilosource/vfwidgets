...
RGBA_COLOR_PATTERN = re.compile(
    r"^rgba\(\s*([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\s*,\s*([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\s*,\s*([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\s*,\s*(0|1|0\.[0-9]+)\s*\)$"
)
...
class Theme:
    ...
    def __post_init__(self) -> None:
        ...
        if self.type not in ("light", "dark", "high-contrast"):
            raise InvalidThemeFormatError(
                f"Invalid theme type: {self.type}. Must be 'light', 'dark', or 'high-contrast'"
            )
    ...
    def _is_valid_color(self, color: str) -> bool:
        ...
        return (
            HEX_COLOR_PATTERN.match(color)
            or RGB_COLOR_PATTERN.match(color)
            or RGBA_COLOR_PATTERN.match(color)
            or HSL_COLOR_PATTERN.match(color)
    ...
    def get_property(
        self, key: PropertyKey, fallback: Optional[PropertyValue] = None
    ) -> PropertyValue:
    ...
    def get_size_estimate(self) -> int:
        ...
        return (
            sys.getsizeof(self)
            + sys.getsizeof(self.colors)
            + sum(sys.getsizeof(k) + sys.getsizeof(v) for k, v in self.colors.items())
            + sys.getsizeof(self.styles)
            + sum(sys.getsizeof(k) + sys.getsizeof(v) for k, v in self.styles.items())
            + sys.getsizeof(self.metadata)
            + sum(sys.getsizeof(k) + sys.getsizeof(v) for k, v in self.metadata.items())
...
class ThemeValidator:
    ...
    def _validate_basic_structure(self, data: Dict[str, Any]) -> bool:
        ...
        if not isinstance(data, dict):
            self._errors.append(
                {"field": "structure", "message": "Theme data must be a dictionary"}
            )
            return False
    ...
    def _validate_name(self, name: Any) -> bool:
        """Validate theme name."""
        if not name:
            self._errors.append({"field": "name", "message": "Theme name is required"})
            return False

        if not isinstance(name, str):
            self._errors.append({"field": "name", "message": "Theme name must be a string"})
            return False

        if not name.strip():
            self._errors.append({"field": "name", "message": "Theme name cannot be empty"})
            return False
    ...
    def _validate_colors(self, colors: Dict[str, Any]) -> bool:
        ...
        for key, value in colors.items():
            if not isinstance(key, str):
                self._errors.append(
                    {"field": f"colors.{key}", "message": "Color keys must be strings"}
                )
                return False

            if not self._is_valid_color(value):
                self._errors.append(
                    {"field": f"colors.{key}", "message": f"Invalid color format: {value}"}
                )
                return False
    ...
    def _validate_token_colors(self, token_colors: List[Any]) -> bool:
        ...
        for i, token in enumerate(token_colors):
            if not isinstance(token, dict):
                self._errors.append(
                    {"field": f"tokenColors[{i}]", "message": "Token color must be a dictionary"}
                )
                return False

            if "scope" not in token:
                self._errors.append(
                    {"field": f"tokenColors[{i}]", "message": 'Token color must have "scope" field'}
                )
                return False
    ...
    def validate_semantic(self, theme: "Theme") -> List[str]:
        ...
        if theme.name == "dark" and theme.type != "dark":
            issues.append(f"Theme named 'dark' should have type='dark', got type='{theme.type}'")

        if theme.name == "light" and theme.type != "light":
            issues.append(f"Theme named 'light' should have type='light', got type='{theme.type}'")

        # Check for missing common tokens (using namespaced keys)
        common_tokens = ["colors.background", "colors.foreground", "colors.primary"]
    ...
    def validate_accessibility(self, theme: Theme) -> ValidationResult:
        ...
        if btn_bg and btn_fg:
            ...
            if btn_ratio < 3.0:
                warnings.append(f"Button contrast ratio {btn_ratio:.2f}:1 is below minimum (3:1)")
                suggestions.append(
        ...
        if bg and focus_border:
            ...
            if focus_ratio < 3.0:
                ...
                suggestions.append("Increase contrast for focus border against background")
        ...
        for state in ["error", "warning", "success"]:
            ...
            if bg and state_color:
                ...
                if state_ratio < 3.0:
                    ...
                    suggestions.append(f"Increase contrast for {state} color against background")
