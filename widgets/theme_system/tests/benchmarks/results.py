...
class BenchmarkResults:
    ...
    def compare_benchmarks(self, benchmark_names: List[str], days: int = 30) -> Dict[str, Any]:
        ...
        if comparison:
            ...
            return {
                ...
                'performance_gap': worst_performer[1]['avg_time_ms'] / best_performer[1]['avg_time_ms']
            }
        else:
    ...
    def _calculate_health_score(self, summary: Dict[str, Any], trends: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self,
        ...
        for name, trend_data in trends.items():
            ...
            if trend_data['time_trend'] > 0.00001:  # Getting slower
                recommendations.append(f"Performance regression detected in {name} - investigate recent changes")

            if trend_data['time_stability'] > 0.2:  # Unstable performance
                recommendations.append(f"Unstable performance in {name} - check for environmental factors")
        ...
        try:
            ...
            with sqlite3.connect(self.results_db) as conn:
                ...
                with open(output_path, 'w', newline='') as csvfile:
                    ...
                    writer.writerow([
                        'Timestamp', 'Duration', 'Iterations', 'Min Time',
                        'Max Time', 'Mean Time', 'Median Time', 'Std Dev', 'Memory Usage'
                    ])
