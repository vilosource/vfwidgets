    ...
    def test_theme_application_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            if results["widget_count"] > 0:
                results["average_time_per_widget"] = results["total_time"] / (
                    results["widget_count"] * len(self.themes)
                )
        ...
        try:
            ...
            for i in range(results["thread_count"]):
                thread = threading.Thread(
                    target=theme_change_worker, args=(i, results["operations_per_thread"])
                )
    ...
    def run_full_scenario(self) -> dict[str, Any]:
        ...
        try:
            ...
            scenario_results["overall_success"] = (
                ...
                and len(scenario_results["errors"]) == 0
                and performance_results.get("widget_count", 0) > 100
                and memory_results.get("memory_test_passed", False)
            )
...
if __name__ == "__main__":
    ...
    if results["concurrent_results"]:
        ...
        print(
            f"  Operations: {conc.get('successful_operations', 0)}/{conc.get('total_operations', 0)}"
        )
