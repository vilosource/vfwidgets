...
def demonstrate_task_16_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        test_theme = Theme(
            ...
            metadata={
                "description": "A demonstration theme for Phase 3",
                "author": "Phase 3 Demo"
            }
        )
        ...
        print(f"  - Save current theme: persistence.save_theme(app.current_theme)")
        print(f"  - Load theme: app.set_theme(persistence.load_theme(path))")
        print(f"  - List available themes: persistence.list_themes()")
...
def demonstrate_task_17_vscode_import():
    ...
    from vfwidgets_theme.importers import VSCodeImporter, VSCodeColorMapper, TokenColorMapper
    ...
    print(f"Dark theme inference: {dark_theme.type} (from {dark_theme_data['colors']['editor.background']})")
    ...
    print(f"Light theme inference: {light_theme.type} (from {light_theme_data['colors']['editor.background']})")
...
def demonstrate_task_18_hot_reload():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"  Failed reloads: {error_stats['total_reloads'] - error_stats['successful_reloads']}")
...
def demonstrate_task_19_theme_factory():
    ...
    from vfwidgets_theme.factory import ThemeFactory, ThemeBuilder, ThemeComposer, ThemeVariantGenerator
    ...
    custom_theme = (factory.create_builder("custom_app_theme")
    ...
    web_theme = (factory.create_from_template("web_app", "my_web_app")
    ...
    material_theme = (factory.create_from_template("material", "material_design_app")
    ...
    # Create base theme
    base_theme = (factory.create_builder("base_theme")
    ...
    # Create accent theme
    accent_theme = (factory.create_builder("accent_theme")
        .add_color("primary", "#007acc")
        .add_color("accent", "#ff6b35")
        .add_style("button", {
            "background-color": "@colors.primary",
    ...
    typography_theme = (factory.create_builder("typography_theme")
        .add_style("window", {
    ...
    # Theme inheritance
    parent_theme = (factory.create_builder("parent_theme")
        .add_color("primary", "#007acc")
    ...
    child_theme = (factory.create_builder("child_theme")
    ...
    cloned_theme = (cloned_builder
        .add_color("test", "#654321")  # Override
        .add_color("new", "#abcdef")   # Add new
        .build())

    print(f"Original builder would have: test=#123456")
    ...
    start_time = time.perf_counter()
    for i in range(10):
        simple_theme = (factory.create_builder(f"perf_test_{i}")
            .add_color("primary", "#007acc")
            .add_color("background", "#ffffff")
    ...
    start_time = time.perf_counter()
    for i in range(5):
        complex_theme = (factory.create_from_template("material", f"complex_test_{i}")
            .add_colors({
                f"custom_{j}": f"#ff{j:04x}" for j in range(10)
            })
            .add_styles({
                f"style_{j}": {"property": f"value_{j}"} for j in range(5)
            })
            .build())
    ...
    gaming_theme = (factory.create_from_template("gaming", "cyberpunk_game")
    ...
    # Test validation
    try:
        invalid_theme = (factory.create_builder("invalid")
            .add_color("", "#ffffff")  # Invalid: empty key
            .build())
    except Exception as e:
        print(f"✓ Validation caught empty color key: {type(e).__name__}")

    try:
        invalid_theme2 = (factory.create_builder("invalid2")
    ...
    lenient_theme = (factory.create_builder("lenient")
