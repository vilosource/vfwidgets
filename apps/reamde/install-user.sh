#!/bin/bash
# Reamde User-Level Installation Script
# Installs Reamde to user directories with full GNOME desktop integration
...
# Find the binary
BINARY=""
if [ -f "$SCRIPT_DIR/Reamde.bin" ]; then
    BINARY="$SCRIPT_DIR/Reamde.bin"
elif [ -f "$SCRIPT_DIR/Reamde" ]; then
    BINARY="$SCRIPT_DIR/Reamde"
else
    echo "❌ Error: Reamde binary not found"
    echo "   Expected: Reamde.bin or Reamde"
...
DESKTOP_FILE="$SCRIPT_DIR/reamde.desktop"
DESKTOP_INSTALL_PATH="$HOME/.local/share/applications/reamde.desktop"
ICON_SVG="$SCRIPT_DIR/icons/reamde.svg"
...
if [ ! -f "$ICON_SVG" ]; then
    ...
    echo "   Please ensure icons/reamde.svg exists"
    exit 1
fi
...
install -Dm644 "$ICON_SVG" "$ICON_DIR/scalable/apps/reamde.svg"
echo "  ✅ Scalable: $ICON_DIR/scalable/apps/reamde.svg"

# Step 3: Install PNG icons if available
for size in 16 22 24 32 48 64 128 256 512; do
    PNG_ICON="$SCRIPT_DIR/icons/reamde-${size}.png"
    if [ -f "$PNG_ICON" ]; then
        mkdir -p "$ICON_DIR/${size}x${size}/apps"
        install -Dm644 "$PNG_ICON" "$ICON_DIR/${size}x${size}/apps/reamde.png"
        echo "  ✅ ${size}x${size}: $ICON_DIR/${size}x${size}/apps/reamde.png"
    fi
done
echo ""
...
echo "=========================================="
echo ""
echo "Reamde has been installed for your user:"
echo "  • Binary:       $BIN_PATH"
echo "  • Desktop file: $DESKTOP_INSTALL_PATH"
echo "  • Icons:        $ICON_DIR/*/apps/reamde.*"
echo ""
echo "You can now:"
echo "  1. Launch from GNOME Activities (search for 'Reamde')"
echo "  2. Run from terminal: reamde"
